...
for i = 1: length(plot_list)
    ...
    title(append(cname,'Test Data Predicted Vs Observed Axial Strain ',plot_list(i),', R^2 = ',string(table2array(M_xgb_50( find( contains(experiment_name,plot_list(i)) ),3)) ) ));
...
plot_list = ["GRS03","ETNA01","MONZ05"]; %experiment_name = ["FBL01","FBL02","ETNA01","ETNA02","MONZ04","MONZ05","WG01","WG02","WG04","GRS02","GRS03","ANS02","ANS03","ANS04","ANS05"];
for i = 1: length(plot_list)
    ...
    title(append(cname,'Predicted Vs Observed Axial Strain ',plot_list(i),', R^2 = ',string(table2array(M_xgb_50( find( contains(experiment_name,plot_list(i)) ),3)) ) ));
    if i == 1
...
for i = 1: length(rads)
    ...
    errorbar(ep_list2,pred_means2,pred_stdev2,'-s','MarkerSize',5,'MarkerFaceColor',[1,0,0],'Color',[1,0,0],'LineWidth',1.3); %[0.8 0.2 0.7]
    if rads(i) == 2
    title(append(cname,'Predicted Vs Observed Axial Strain ',plot_list,', R^2 = ',string(table2array(M_xgb_20( find( contains(experiment_name,plot_list) ),3)) ) ));
    else
    title(append(cname,'Predicted Vs Observed Axial Strain ',plot_list,', R^2 = ',string(table2array(M_xgb_50( find( contains(experiment_name,plot_list) ),3)) ) ));  
    end
    if i == 1
    legend('Model Prediction','Observed Strain','Mean Model Output','Location','northwest');
    end 
